...
INC_DIRS += ${LIBFT_INC}
LIBS += -L${LIBFT_ROOT} -lft -L.local/lib -lreadline
...
DIRS := ./:utilities/:word_expansion/:utilities/debugger/:signals/:parser/:execution/:cli/:builtins/:parser/token/
...
obj/asan/asan.o: sources/asan/asan.c
	${AT}mkdir -p ${@D} ${BLOCK}
	${AT}${CC} -o $@ -c $< ${BLOCK}
